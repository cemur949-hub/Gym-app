...
import { hexColor, type WorkoutProgram, type Workout } from '../types'
        ...
        <div className="px-4 pt-4 pb-2 safe-top shrink-0">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            <button onClick={() => setFilterSplitID('all')}
              ...
              style={{ background: filterSplitID === 'all' ? hexColor(settings.accentColorHex) : '#222', color: filterSplitID === 'all' ? '#fff' : '#8E8E93' }}>
              All
            </button>
            {program.splits.map(s => (
              <button key={s.id} onClick={() => setFilterSplitID(filterSplitID === s.id ? 'all' : s.id)}
                className="px-3 py-1 rounded-full text-xs font-bold shrink-0"
                style={{ background: filterSplitID === s.id ? `#${s.colorHex}` : '#222', color: filterSplitID === s.id ? '#fff' : '#8E8E93' }}>
                {s.icon} {s.name}
              </button>
            ))}
          </div>
          ...
          {filteredWorkouts.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <button onClick={() => setEditingWorkout('new')}
                ...
                style={{ background: hexColor(settings.accentColorHex), color: '#fff' }}>
        ...
        <div className="fixed inset-0 z-40 flex flex-col" style={{ background: '#0A0A0A' }}>
          <div className="flex items-center justify-between px-4 py-3 border-b border-divider safe-top">
            ...
            <button onClick={() => { setShowSplits(false); setShowCreateSplit(true) }}
              className="text-sm font-bold" style={{ color: hexColor(settings.accentColorHex) }}>
              + New
            </button>
          </div>
