...
struct SettingsView: View {
    ...
    var body: some View {
        ...
        .alert("All Data Cleared", isPresented: $showImportSuccess) {
            Button("OK") {}
        } message: {
            Text("Workouts imported successfully.")
        }
        ...
        .alert("Clear All Data?", isPresented: $showClearConfirm) {
            ...
            Text("This will permanently delete all programs and workouts.")
        }
    }
    ...
    private var appHeaderSection: some View {
        HStack(spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text("\(store.programs.count) programs · \(totalWorkouts) workouts")
                    .font(.caption)
                    .foregroundColor(.textSecondary)
            }
            Spacer()
    ...
    private var totalWorkouts: Int {
        store.programs.reduce(0) { $0 + $1.workouts.count }
    }
    ...
    private var appearanceSection: some View {
        SettingsSection(title: "Appearance") {
            ...
            SettingsRow(
                icon: "timer",
                iconColor: .blue,
                label: "Show Rest Times"
            ) {
                Toggle("", isOn: $settings.showRestTimes)
    ...
    private var generalSection: some View {
        SettingsSection(title: "Defaults") {
            SettingsRow(icon: "tag.fill", iconColor: .purple, label: "Default Split") {
                Menu {
                    ForEach(SplitType.allCases) { split in
                        Button {
                            settings.defaultSplit = split
                        } label: {
                            Label(split.rawValue, systemImage: split.icon)
                        }
                    }
                } label: {
                    HStack(spacing: 4) {
                        Text(settings.defaultSplit.rawValue)
                            .foregroundColor(.textSecondary)
